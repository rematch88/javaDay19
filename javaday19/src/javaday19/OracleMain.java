...
public class OracleMain {
	...
	public static void main(String[] args) {
		try (BufferedReader br = new BufferedReader(
			...
			Calendar cal = new GregorianCalendar(2020, 0, 31, 14, 24, 00);
			//java.sql.Date
			Date transdate = new Date(cal.getTimeInMillis());
			pstmt.setDate(6, transdate);
			
			pstmt.setString(7, "아놀드홍");
			...
			*/
			...
			PreparedStatement pstmt =
					con.prepareStatement(
							"update transactions set itemcode=?, itemname=?, price=?, cnt=?, transdate=?, userid=?"
									+ "where num=?");
			...
			*/
			...
			System.out.println(BCrypt.checkpw("터미네이터", BCrypt.hashpw("터미네이터1", BCrypt.gensalt())));
			
			System.out.println(BCrypt.hashpw("터미네이터", BCrypt.gensalt()));
			System.out.println(BCrypt.hashpw("터미네이터", BCrypt.gensalt()));
			
			System.out.println(BCrypt.checkpw("터미네이터", 
					"$2a$10$pQ7K2/H/DgVmiHdw4k4KIevu3K1dvnoKAo9NeLO/aboM1DS8HMCNW"));
			System.out.println(BCrypt.checkpw("터미네이터", 
					"$2a$10$jl3sX2vdJj5CH1.7MskEZehrgON68Y8QupxMLcVK0iUgggPCG2UAS"));
			*/
			...
			//id나 검색어 등은 모두 대문자 또는 모두 소문자로 변경해서 저장하는 것이 일반적
			...
			*/
			...
			//commit()을 만나거나 DDL, DCL을 성공할 때까지 데이터베이스에 반영이 안됩니다.
